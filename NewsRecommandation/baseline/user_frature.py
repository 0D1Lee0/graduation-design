...
train_df = pd.read_csv(save_path + 'train_04-11.csv')
test_df = pd.read_csv(save_path + 'test_04-11.csv')
val_df = pd.read_csv(save_path + 'val_04-11.csv')
print(len(train_df))
all_data = train_df.append(val_df)
print(len(all_data))
    ...
    for user_id in tqdm(users_id):
        ...
        for rank, (article_id, score, label) in enumerate(recall_list[user_id]):
            ...
            for hist_item in hist_user_items:
                ...
                #print("type(hist_item)="+str(type(hist_item)) + "    type(article_id)="+str(type(article_id)))
                #print("hist_item=" + str(hist_item) + "    article_id=" + str(article_id))
                article_id_ = int(article_id)
                #print("type(hist_item)="+str(type(hist_item)) + "    type(article_id_)="+str(type(article_id_)))
                #print("hist_item=" + str(hist_item) + "    article_id_=" + str(article_id_))
...
def merge_info(all_data, device_cols, user_time_hob_cols, user_category_hob_cols, articles, articles_cols):
    ...
    user_info.to_csv(save_path + 'user_info'+'_'+ datetime.today().strftime('%m-%d') + '.csv', index=False)
    print("用户信息保存成功！")
...
if __name__ == '__main__':
    ...
    merge_info(all_data, device_cols, user_time_hob_cols, user_category_hob_cols, articles, articles_cols)
