...
def get_user_features(raw_data, train_dataset, test_users, articles_dic):
    ...
    def calc_lag_between_two_articles(items, articles_dic):
        if len(items) > 1:
            return (articles_dic[items[-1][0]]['created_at_ts'] - articles_dic[items[-2][0]]['created_at_ts']) / (
                        1000 * 60 * 60 * 24)
        else:
            return np.nan
    ...
    for user_id, ts_set in tqdm(test_users.items()):
        for last_clicked_timestamp in ts_set:
            _data.append((
                ...
                calc_lag_between_created_at_ts_and_clicked_ts(train_dataset[user_id][last_clicked_timestamp],
                                                              articles_dic),
    ...
    df1 = pd.DataFrame(_data, columns=['user_id', 'last_clicked_timestamp', 'avg_words_count', 'min_words_count',
                                       'max_words_count', 'lag_between_created_at_ts_and_clicked_ts',
                                       'lag_between_two_click', 'lag_between_two_articles'])

    # 计算用户使用设备，环境等的众数
    columns = ['user_id', 'click_environment', 'click_deviceGroup', 'click_os', 'click_country', 'click_region',
               'click_referrer_type']
...
def create_train_data(raw_data, train_dataset, test_users, articles_dic, recall_results, offline, y_answer):
    ...
    for user_id, ts_set in test_users.items():
        for last_clicked_timestamp in ts_set:
            ...
            keys_ds.append(
                list(zip(np.repeat(user_id, len(items)), np.repeat(last_clicked_timestamp, len(items)), items)))

    ds = pd.DataFrame(np.concatenate(keys_ds), columns=['user_id', 'last_clicked_timestamp', 'article_id'],
                      dtype=np.int64).drop_duplicates()
    ...
    if offline:
        ...
        answers = pd.DataFrame(answer_keys_ds, columns=['user_id', 'last_clicked_timestamp', 'article_id'],
                               dtype=np.int64)
        # 将正确答案融合进数据集
        answers['answer'] = 1
