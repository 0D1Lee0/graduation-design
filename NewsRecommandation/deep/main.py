...
def read_all_raw_data(
        filenames=['articles.csv', 'train_click_log.csv', 'testB_click_log_Test_B.csv', 'testA_click_log.csv']):
...
def test():
    ...
    clf.fit(_X_train, y_train, group=X_grp_train, eval_set=[(handle_columns(X_test), y_test)], eval_group=[X_grp_test],
            eval_at=[1, 2, 3, 4, 5], eval_metric=['ndcg', ], early_stopping_rounds=50, verbose=False)
    print('Best iteration: {}'.format(clf.best_iteration_))

    for X, X_grp, df, title in [(X_test, X_grp_test, df_test, 'Test Set'),
                                (X_val, X_grp_val, df_val, 'Validation Set')]:
        print('[{}]'.format(title))
...
def run():
    ...
    clf.fit(_X_train, y_train, group=X_grp_train, eval_set=[(handle_columns(X_eval), y_eval)], eval_group=[X_grp_eval],
            eval_at=[1, 2, 3, 4, 5], eval_metric=['ndcg', ], early_stopping_rounds=50, verbose=False)
    print('Best iteration: {}'.format(clf.best_iteration_))
