...
data_path = 'E:/recommend_data/aliyun/original_data/'
save_path = 'E:/recommend_data/aliyun/result_data/'
...
def get_all_click_df(data_path='E:/recommend_data/aliyun/original_data/', train=True, test=True):
    if train:
...
def hot_recall(topk=10, train_past_clicks=None, test_last_click=None):
    ...
    for user_id in tqdm(train_past_clicks['user_id'].unique()):
        ...
        item_topk_click = get_item_topk_click_(train_hot_articles, train_hot_articles_dict, click_time,
                                               past_click_articles, k=topk)
    ...
    for user_id in tqdm(test_click_df['user_id'].unique()):
        ...
        past_click_articles = user['click_article_id'].values
        item_topk_click = get_item_topk_click_(test_hot_articles, test_hot_articles_dict, click_time,
                                               past_click_articles, k=topk)
        for id in item_topk_click:
            rows = [user_id, id]
...
def get_train_recall(itemcf=False, hot=False, train_last_click=None):
    ...
    if hot:
        ...
        hot_train_recall['label'] = hot_train_recall.merge(train_last_click, on=['user_id', 'article_id'], how='left')[
            'click_timestamp'].apply(lambda x: 0.0 if np.isnan(x) else 1.0)
    ...
    def neg_sample(train=None):
        ...
        def neg_sample_recall_data(recall_items_df, sample_rate=0.001):
            ...
            print('pos_data_num:', len(pos_data), 'neg_data_num:', len(neg_data), 'pos/neg:',
            ...
            neg_data_new = neg_data_new.sort_values(['user_id', 'pred_score']).drop_duplicates(
                ['user_id', 'article_id'], keep='last')
...
def train_and_predict(itemcf=False, itemcf_topk=10, hot=False, hot_topk=10, offline=True):
    ...
    train = train.drop(columns=['click_environment', 'click_deviceGroup', 'click_os', 'click_country', 'click_region',
                                'click_referrer_type']).merge(
        train_last_click.drop(columns=['article_id', 'click_timestamp']))
    ...
    lgb_cols = ['click_environment', 'click_deviceGroup', 'click_os', 'click_country',
                'click_region', 'click_referrer_type', 'category_id', 'created_at_ts', 'words_count', 'click_timestamp',
                'delta_time']
    ...
    lgb_ranker.fit(X_train[lgb_cols], y_train, group=g_train, eval_set=[(X_eval[lgb_cols], y_eval)],
                   eval_group=[g_eval], early_stopping_rounds=50, verbose=False)
    ...
    X_off = X_off.drop(
        columns=['category_id', 'created_at_ts', 'words_count', 'click_environment', 'click_deviceGroup', 'click_os',
                 'click_country', 'click_region', 'click_referrer_type', 'click_timestamp', 'delta_time'])
    ...
    if not offline:
        ...
        result = result.drop(
            columns=['category_id', 'created_at_ts', 'words_count', 'click_environment', 'click_deviceGroup',
...
if __name__ == '__main__':
    if len(sys.argv) == 4:
        ...
        if sys.argv[1] == 'train':
            ...
            offline = False
        train_and_predict(itemcf=True, itemcf_topk=int(sys.argv[3]), hot=True, hot_topk=int(sys.argv[5]),
                          offline=offline)
    else:
        print('Wrong command.')